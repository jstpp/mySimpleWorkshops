    ...
    if(isset($_GET['call']) and is_logged_in())
    {
        try {
            ...
            {    
                ...
                $db = $pdo->prepare("SELECT * FROM courses WHERE id=:course_id FOR UPDATE");
                $db->execute(['course_id' => $_GET['id']]);
                $course = $db->fetch();
                if (!isset($course['id'])) {
                    throw new Exception("Kurs nie został znaleziony.");
                }

                $db = $pdo->prepare("SELECT * FROM signins WHERE course_id=:course_id AND user_id=:user_id");
                $db->execute(['course_id' => $_GET['id'], 'user_id' => $_SESSION['user']['id']]);
                if ($db->fetch()) {
                    throw new Exception("Jesteś już zapisanx.");
                }
            ...
            } else if ($_GET['call']=="sign_out" and isset($_GET['id'])) {
                $pdo->beginTransaction();
            ...
            } else if ($_GET['call']=="remove_round" and is_admin() and isset($_GET['id'])) {
                $pdo->beginTransaction();
                ...
                redirect("index.php?mode=management");
            } else if ($_GET['call']=="remove_course" and is_admin() and isset($_GET['id'])) {
                $pdo->beginTransaction();
                ...
                $pdo->commit();
                redirect("index.php?mode=management");
            } else if ($_GET['call']=="modify_course" and is_admin() and isset($_GET['id'])) {
                $db = $pdo->prepare("SELECT * FROM courses WHERE id=:course_id FOR UPDATE");
                $db->execute(['course_id' => $_GET['id']]);
                $course = $db->fetch();
                if (!isset($course['id'])) {
